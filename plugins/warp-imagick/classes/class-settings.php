...
if ( ! class_exists( __NAMESPACE__ . '\Settings' ) ) {
	...
	# region Settings Extended
		...
		public function check_conflicting_plugins() {
			...
			foreach ( $conflicts_active as $conflict_path => $conflict_slug ) {
				...
				$conflict_pl_name = $conflict_slug;
				$conflict_message = "Plugin https://wordpress.org/plugins/$conflict_slug/ or Warp iMagick may not work properly when used along.";
				...
				switch ( $conflict_slug ) {
					...
					default:
						...
						$conflict_message = "Plugin '$conflict_pl_name' (https://wordpress.org/plugins/$conflict_slug/) or Warp iMagick, may not work properly when used along.";
						break;

				}
				$conflict_message = "Warp iMagick - Known conflict detected:\n" . trim( $conflict_message ) . "\nSee 'Known conflicts' section in Warp iMagick Readme.txt file (or read https://wordpress.org/plugins/{$this->pageslug}) for more details.";
	...
	# region Plugin Manager
		...
		protected function on_check_activate_requirements( $networkwide, $settings ) {
			...
			if ( ! empty( $fail ) ) {
				$fail [] = 'Please read plugin FAQ and ask your host service provider for help to install or upgrade PHP-imagick module linked with ImageMagick library version 6';
			}
